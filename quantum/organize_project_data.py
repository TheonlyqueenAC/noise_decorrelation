...
# Using Path.home() to avoid IDE warnings and make it portable
# This assumes standard Mac directory structure: ~/Documents/Github/...
# If your repo is elsewhere, modify the path components after Path.home()
LOCAL_REPO = Path.home() / "Documents" / "Github" / "noise_decorrelation_HIV" / "data"

# Define data directory structure
# LOCAL_REPO already points to the data/ directory
DATA_DIRS = {
    "raw": LOCAL_REPO / "raw",  # Original tables/Excel
    "extracted": LOCAL_REPO / "extracted",  # Study-specific CSVs
    "master": LOCAL_REPO / "master",  # Integrated databases
    "model_outputs": LOCAL_REPO / "model_outputs",  # Bayesian results
    "individual": LOCAL_REPO / "individual",  # Patient-level data (Valcour)
}
...
"""
    ...
    inventory_path = LOCAL_REPO / "DATA_INVENTORY.md"
...
def main():
    ...
    print("=" * 60)
    print(f"\nData organized in: {LOCAL_REPO}")
    print(f"\nNext steps:")
    print(f"1. cd {LOCAL_REPO.parent}")  # Go to repo root
