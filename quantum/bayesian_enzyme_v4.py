...
# Optional heavy deps (PyMC, ArviZ) — loaded lazily to keep import light
try:
    import pymc as pm  # type: ignore
except Exception:  # pragma: no cover
    pm = None  # will be checked at runtime where needed
...
# Optional imports (plotting); make non-fatal if not installed
try:
    import matplotlib.pyplot as plt
except Exception:  # pragma: no cover
    plt = None

# Tensor ops inside PyMC: prefer PyTensor (PyMC v5+), fallback to Aesara
try:
    ...
    try:
        ...
        at = None
        TensorVariable = tuple()

# Import enzyme kinetics module (relative within package, fallback to path insert)
import sys as _sys
...
def forward_model_enzyme(xi_acute, xi_chronic, beta_xi, gamma_coh,
    ...
    Parameters
    ----------
    xi_acute : float or tensor
    xi_chronic : float or tensor
    beta_xi : float or tensor
    gamma_coh : float or tensor
    viral_damage_acute : float or tensor
    viral_damage_chronic : float or tensor
    membrane_acute : float or tensor
    membrane_chronic : float or tensor
    coh_acute : float
    coh_chronic : float

    Returns
    -------
    NAA_pred : length-3 array/tensor
        [NAA_healthy, NAA_acute, NAA_chronic] in MRS units
    Cho_pred : length-3 array/tensor
    ...
    # Stack results; use Aesara stack if available (inside PyMC graph)
    if at is not None and any(getattr(x, 'owner', None) is not None for x in [NAA_h, NAA_a, NAA_c] if hasattr(x, 'owner')):
        NAA_vec = at.stack([NAA_h, NAA_a, NAA_c])
        Cho_vec = at.stack([Cho_h, Cho_a, Cho_c])
        NAA_pred = NAA_vec / creatine
        Cho_pred = Cho_vec / creatine
    else:
...
def build_enzyme_model():
    ...
    if pm is None:
        raise ImportError("PyMC is not installed. Please install pymc>=4 to build the model.")
...
def run_inference(n_samples=2000, n_chains=4, target_accept=0.99):
    ...
    if pm is None:
        raise ImportError("PyMC is not installed. Please install pymc>=4 to run inference.")
...
def analyze_results(idata):
    ...
    if az is None:
        print("ArviZ is not available; skipping detailed analysis summary.")
        return None
...
def plot_results(idata, output_dir='results/enzyme_v4'):
    ...
    # Require ArviZ for plotting posteriors
    if az is None:
        print("ArviZ not available; skipping posterior plots.")
    else:
