...
# Runtime guard for NumPy 2.x incompatibility with PyTensor/BLAS used by PyMC
try:
...
if _np_major >= 2:
    msg = (
        "Detected NumPy %s. PyTensor (PyMC backend) expects NumPy 1.x config API.\n"
        "Fix: activate your venv and install a compatible NumPy: pip install \"numpy<2.0\"\n"
        "Or run: make install (uses requirements.txt pins)." % np.__version__
    )
    print(msg, file=sys.stderr)
    sys.exit(1)

# PyMC v4 (guarded import; provide clearer hint on BLAS config error)
try:
    import pymc as pm
    ...
    emsg = str(e)
    if "numpy.__config__.get_info" in emsg or "get_info" in emsg or "blas__ldflags" in emsg:
        print(
            "PyTensor/BLAS initialization failed (likely due to incompatible NumPy).\n"
            "Fix: pip install \"numpy<2.0\" in your venv, or run: make install",
            file=sys.stderr,
        )
        sys.exit(1)
    # Unknown import error; re-raise for visibility
    raise
...
def forward_NAA_compensated(coherence_base: float,
    ...
    # 4. Adaptive homeostatic floor (chronic only):
    # In chronic phase, apply a permissive floor at ~85% of healthy to reflect
    # compensatory biology without forcing floor activation in all conditions.
    if condition == 'chronic_HIV':
...
def run_inference_v2(draws: int = 3000, 
    ...
    for cond in CONDITIONS:
        ...
        # Adaptive homeostatic floor: apply only in chronic at ~85% of healthy
        if cond == "chronic_HIV":
            naa = max(naa, 0.85 * CONST.NAA_baseline)
