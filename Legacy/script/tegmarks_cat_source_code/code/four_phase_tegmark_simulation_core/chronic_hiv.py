#!/usr/bin/env python3
"""
Terminal AIDS Simulation Module
-----------------------------
Simulates the effects of chronic untreated HIV (terminal AIDS) on quantum coherence in microtubules.

This module models the cytokine distribution characteristic of terminal AIDS with elevated
cytokine storm patterns and simulates their effects on quantum coherence preservation in
regular vs. Fibonacci-scaled spatial grids.
"""
...
def initialize_terminal_cytokine_field(r, z, R_inner, R_outer, N_z, N_r):
    """
    Initialize cytokine field for terminal AIDS.

    Creates a spatially varying cytokine concentration field characteristic
    of terminal AIDS, with extreme intensity and widespread cytokine storm patterns.
    ...
    """
    log_message("Initializing terminal AIDS cytokine field")
    cytokines = np.zeros((N_z, N_r))
    L = np.max(z)

    # Terminal AIDS pattern: extreme intensity with cytokine storm
    for i in range(N_z):
        for j in range(N_r):
            ...
            # Terminal AIDS pattern: intense and widespread
            cytokines[i, j] = 0.9 * np.exp(-center_dist ** 2 / 2.5)

    # Add high-intensity background representing systemic inflammation
    cytokines += 0.4 * np.random.random((N_z, N_r))

    # Add multiple intense cytokine storm hotspots
    num_spots = 12  # More hotspots than other conditions
    for _ in range(num_spots):
        i = np.random.randint(0, N_z)
        j = np.random.randint(0, N_r)
        size = 6 + np.random.randint(4)  # Larger hotspots
        ...
        for di in range(-size, size + 1):
            for dj in range(-size, size + 1):
                ...
                if 0 <= ii < N_z and 0 <= jj < N_r:
                    ...
                    if dist <= 1:
                        intensity = 0.6 * np.exp(-2 * dist ** 2)  # Higher intensity
                        cytokines[ii, jj] = min(1.0, cytokines[ii, jj] + intensity)

    # Add chaotic wave patterns characteristic of dysregulated immune response
    for i in range(N_z):
        for j in range(N_r):
            wave = 0.2 * np.sin(5 * np.pi * z[i] / L) * np.cos(6 * np.pi * r[j] / (R_outer - R_inner))
            cytokines[i, j] += np.abs(wave)

    # Normalize to [0, 1] with high minimum representing systemic baseline inflammation
    cytokines = np.clip(cytokines, 0.3, 1.0)  # Minimum 0.3 baseline inflammation

    return cytokines


def update_cytokine_field(C, dt, current_time, dr, dz):
    """
    Update cytokine field over time for terminal AIDS simulation.
    ...
    # Terminal AIDS pattern: chaotic oscillations with cytokine storms
    time_factor = 1.0 + 0.4 * np.sin(1.5 * current_time) * np.cos(0.7 * current_time)

    # Random cytokine storm bursts
    if np.random.random() < 0.05:  # 5% chance of cytokine storm burst
        storm_factor = 1.5  # Strong amplification
        storm_duration = 0.1  # Short duration
        time_factor *= storm_factor * np.exp(-current_time % 1.0 / storm_duration)

    # Diffusion and decay rates
    diffusion_rate = 0.08  # Rapid diffusion in cytokine storm
    decay_rate = 0.005 * time_factor  # Very slow decay due to impaired clearance

    # Strong fluctuations modeling dysregulated immune response
    fluctuation_amplitude = 0.15
    fluctuations = fluctuation_amplitude * (np.random.random(C.shape) - 0.3)  # Biased toward increase

    # Update cytokine field
    C_new = C + dt * (diffusion_rate * laplacian_C - decay_rate * C + fluctuations)

    # Add occasional new hotspots
    if np.random.random() < 0.02:  # 2% chance per timestep
        i, j = np.random.randint(0, N_z), np.random.randint(0, N_r)
        ...
        for di in range(-size, size + 1):
            for dj in range(-size, size + 1):
                ...
                if 0 <= ii < N_z and 0 <= jj < N_r:
                    dist = np.sqrt(di ** 2 + dj ** 2) / size
                    if dist <= 1:
                        C_new[ii, jj] = min(1.0, C_new[ii, jj] + intensity * np.exp(-3 * dist ** 2))

    # Maintain high baseline due to systemic inflammation in terminal phase
    C_new = np.clip(C_new, 0.3, 1.0)
...
def run_simulation(config):
    """
    Run the terminal AIDS simulation with the given configuration.
    ...
    try:
        ...
        condition_params = config["conditions"]["terminal"]  # Use terminal condition params
        output_params = config["output"]

        # Setup directories
        results_dir = condition_params.get("output_dir", "results/terminal")
        figures_dir = condition_params.get("figures_dir", "figures/terminal")
        logs_dir = condition_params.get("logs_dir", "logs/terminal")
        ...
        # Setup logging
        log_file = os.path.join(logs_dir, f"terminal_aids_{create_timestamp()}.log")
        ...
        # Simulation parameters specific to terminal AIDS - use higher values
        V_0 = condition_params.get("V_0", 1.5 * sim_params["V_0"])  # 50% stronger
        Gamma_0 = condition_params.get("Gamma_0", 1.8 * sim_params["Gamma_0"])  # 80% stronger
        alpha_c = condition_params.get("alpha_c", 1.3 * sim_params["alpha_c"])  # 30% stronger
        coupling_strength = condition_params.get("IL6_COUPLING_STRENGTH", 0.35)
        ...
        C = initialize_terminal_cytokine_field(r, z, R_inner, R_outer, N_z, N_r)
        ...
        Gamma_reg = calculate_decoherence(C, Gamma_0, alpha_c, "terminal", coupling_strength, grid_type="regular")
        Gamma_fib = calculate_decoherence(C, Gamma_0, alpha_c, "terminal", coupling_strength, grid_type="fibonacci")
        ...
        # Time evolution loop
        log_message("Starting time evolution simulation for terminal AIDS")
        ...
        final_horizon_reg = calculate_event_horizon(Psi_reg_prob, r, coherence_reg_list[-1], grid_type="regular", R_grid=R)
        final_horizon_fib = calculate_event_horizon(Psi_fib_prob, r, coherence_fib_list[-1], grid_type="fibonacci", R_grid=R_fib)
        ...
        # Save to JSON file
        results_file = os.path.join(results_dir, "terminal_aids_results.json")
        with open(results_file, 'w') as f:
            json.dump(results, f, indent=4)
        log_message(f"Results saved to {results_file}")

        # Save time series data to NPZ
        npz_file = os.path.join(results_dir, "terminal_aids_data.npz")
        np.savez(npz_file,
        ...
        log_message("Terminal AIDS simulation completed successfully!")
        return True

    except Exception as e:
        log_message(f"Error in terminal AIDS simulation: {e}")
        import traceback
