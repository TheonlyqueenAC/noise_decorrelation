#!/usr/bin/env python3
"""
ART-Controlled HIV Simulation Module
----------------------------------
Simulates the effects of ART-controlled HIV infection on quantum coherence in microtubules.

This module models the cytokine distribution characteristic of HIV infection
under antiretroviral therapy (ART) control and simulates its effects on quantum
coherence preservation in regular vs. Fibonacci-scaled spatial grids.
"""
...
def initialize_art_cytokine_field(r, z, R_inner, R_outer, N_z, N_r):
    """
    Initialize cytokine field for ART-controlled HIV.

    Creates a spatially varying cytokine concentration field characteristic
    of ART-controlled HIV infection, with low intensity and localized distribution.
    ...
    """
    log_message("Initializing ART-controlled HIV cytokine field")
    cytokines = np.zeros((N_z, N_r))
    L = np.max(z)

    # ART-controlled pattern: low intensity, more localized
    for i in range(N_z):
        for j in range(N_r):
            ...
            # ART-controlled pattern: low intensity, localized
            cytokines[i, j] = 0.3 * np.exp(-center_dist ** 2 / 1.2)

    # Add very low-intensity background
    cytokines += 0.1 * np.random.random((N_z, N_r))

    # Add a few small, low-intensity spots
    num_spots = 4
    for _ in range(num_spots):
        i = np.random.randint(0, N_z)
        j = np.random.randint(0, N_r)
        size = 3 + np.random.randint(3)
        ...
        for di in range(-size, size + 1):
            for dj in range(-size, size + 1):
                ...
                if 0 <= ii < N_z and 0 <= jj < N_r:
                    ...
                    if dist <= 1:
                        intensity = 0.2 * np.exp(-3 * dist ** 2)
                        cytokines[ii, jj] = min(1.0, cytokines[ii, jj] + intensity)

    # Normalize to [0, 1]
    cytokines = np.clip(cytokines, 0, 0.7)  # Lower maximum to represent controlled environment
...
def update_cytokine_field(C, dt, current_time, dr, dz):
    """
    Update cytokine field over time for ART-controlled HIV simulation.
    ...
    # ART-controlled pattern: minimal oscillation, slowly decreasing
    time_factor = 1.0 + 0.05 * np.sin(0.3 * current_time) - 0.02 * current_time
    time_factor = max(0.5, time_factor)  # Ensure non-negative

    # Diffusion and decay rates
    diffusion_rate = 0.01  # Very slow diffusion
    decay_rate = 0.015 * time_factor  # Moderate decay rate due to ART

    # Small fluctuations
    fluctuation_amplitude = 0.003
    ...
    C_new = np.clip(C_new, 0, 0.7)  # Keep within controlled bounds
...
def run_simulation(config):
    """
    Run the ART-controlled HIV simulation with the given configuration.
    ...
    try:
        ...
        condition_params = config["conditions"]["art_controlled"]
        output_params = config["output"]

        # Setup directories
        results_dir = condition_params.get("output_dir", "results/art_controlled")
        figures_dir = condition_params.get("figures_dir", "figures/art_controlled")
        logs_dir = condition_params.get("logs_dir", "logs/art_controlled")
        ...
        # Setup logging
        log_file = os.path.join(logs_dir, f"art_controlled_{create_timestamp()}.log")
        ...
        # Simulation parameters specific to ART-controlled HIV
        V_0 = condition_params["V_0"]
        Gamma_0 = condition_params["Gamma_0"]
        alpha_c = condition_params["alpha_c"]
        ...
        C = initialize_art_cytokine_field(r, z, R_inner, R_outer, N_z, N_r)
        ...
        # Initialize decoherence rates with grid_type parameter
        log_message("Initializing decoherence rates")
        Gamma_reg = calculate_decoherence(C, Gamma_0, alpha_c, "art_controlled", coupling_strength, grid_type="regular")
        Gamma_fib = calculate_decoherence(C, Gamma_0, alpha_c, "art_controlled", coupling_strength,
                                          grid_type="fibonacci")
        ...
        # Time evolution loop
        log_message("Starting time evolution simulation for ART-controlled HIV")
        ...
        # Initial horizon calculation with R_grid parameter
        ...
        for step in range(1, time_steps + 1):
            ...
            V_reg = calculate_potential(R, Z, C, V_0)
            V_fib = calculate_potential(R_fib, Z_fib, C, V_0)
            Gamma_reg = calculate_decoherence(C, Gamma_0, alpha_c, "art_controlled", coupling_strength,
                                              grid_type="regular")
            Gamma_fib = calculate_decoherence(C, Gamma_0, alpha_c, "art_controlled", coupling_strength,
                                              grid_type="fibonacci")

            # Evolve wavefunctions with grid_type parameter for Tegmark decoherence
            ...
            # Calculate coherence with R_grid parameter
            ...
            # Save data at regular intervals based on save_frequency
        ...
        # Calculate final horizon with R_grid parameter
        ...
        results = {
            ...
            'state': "art_controlled",
            'timestamp': datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }

        # Save to JSON file
        results_file = os.path.join(results_dir, "art_controlled_results.json")
        with open(results_file, 'w') as f:
            json.dump(results, f, indent=4)
        log_message(f"Results saved to {results_file}")

        # Save time series data to NPZ
        npz_file = os.path.join(results_dir, "art_controlled_data.npz")
        np.savez(npz_file,
        ...
        log_message("ART-controlled HIV simulation completed successfully!")
        return True

    except Exception as e:
        log_message(f"Error in ART-controlled HIV simulation: {e}")
        import traceback
