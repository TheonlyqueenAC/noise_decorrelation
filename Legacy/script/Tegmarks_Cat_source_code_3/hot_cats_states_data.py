...
class SimulationParameters:
    ...
    def __init__(self):
        ...
        # Parameters from HAND paper
        self.fibonacci_exponent = -1.0150  # Power law exponent for Fibonacci grid
        self.regular_exponent = -10.1023  # Power law exponent for regular grid
...
def calculate_quantum_coherence(time, geometry, temperature):
    """
    Implement key finding from both papers showing different decay patterns.
    ...
    if geometry == 'fibonacci':
        ...
        coherence = initial_coherence * 0.015963 * (time ** params.fibonacci_exponent) * temp_factor
        ...
        coherence = initial_coherence * (time ** params.regular_exponent) * temp_factor
...
def run_core_quantum_dynamics_analysis():
    ...
    # Base coherence values using power law exponents
    # The paper uses power law decay: t^exponent for both types
    # For true normalization, use actual normalized values based on the graphs
    initial_value = 1e12  # Starting value based on graph
    fibonacci_coherence = initial_value * (times ** params.fibonacci_exponent)
    regular_coherence = initial_value * (times ** params.regular_exponent)

    # Calculate coherence ratio
    coherence_ratio = fibonacci_coherence / regular_coherence

    # Determine sanctuary formation time (where regular grid coherence reaches 0.5% of initial)
    sanctuary_threshold = 0.005
    sanctuary_indices = np.where(regular_coherence < sanctuary_threshold * initial_value)[0]
    if len(sanctuary_indices) > 0:
        ...
        print(f"Advantage at sanctuary formation: {advantage_at_sanctuary:.2f}x")
    else:
        sanctuary_time = None
    ...
    print(f"Maximum coherence advantage: {max_ratio:.2f}x at t = {max_time:.2f}")

    # Create figure
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8))

    # Plot coherence evolution
    ax1.loglog(times, regular_coherence, 'b-', label=f'Regular Grid (λ={params.regular_exponent})')
    ax1.loglog(times, fibonacci_coherence, 'r-', label=f'Fibonacci Grid (λ={params.fibonacci_exponent})')

    if sanctuary_time:
        ax1.axvline(x=sanctuary_time, color='g', linestyle='--', label='Sanctuary formation')

    ax1.set_title('Quantum Coherence Dynamics During Acute HIV Inflammation')
    ax1.set_xlabel('Time')
...
def run_golden_ratio_optimization_analysis():
    ...
    plt.tight_layout()
    plt.savefig('golden_ratio_optimization.png', dpi=300)
...
def run_temperature_resilience_analysis():
    ...
    plt.tight_layout()
    plt.savefig('temperature_resilience.png', dpi=300)
...
def run_extended_decay_analysis():
    ...
    dict
        Results of the analysis
    """
    print("\nRunning Extended Multiplicative Decay analysis (0-120 time units)...")
    ...
    # Define decay factors based on paper
    reg_degradation_factor = 0.1  # Regular grid: rapid decay
    fib_degradation_factor = 0.001  # Fibonacci grid: much slower decay
    ...
    # Note: From the graph, the Fibonacci grid maintains nearly constant coherence
    fibonacci_coherence = 0.99 + 0.01 * (1 - fib_degradation_factor) ** times
    ...
    ax1.plot(times, regular_coherence, 'b-', label='Regular Grid')
    ax1.plot(times, fibonacci_coherence, 'r-', label='Fibonacci Grid')
    ax1.set_title('Extended Coherence Analysis (0-120 time units)')
    ax1.set_xlabel('Time')
    ...
    ax2.plot(times, coherence_ratio, 'g-')
    ax2.set_title(f'Coherence Ratio\nFinal advantage: {final_ratio:.2e}')
    ax2.set_xlabel('Time')
...
def simulate_sanctuary_formation(times, resolution=100):
    ...
    plt.savefig('sanctuary_formation.png', dpi=300)
...
def compare_hiv_phases():
    ...
    plt.tight_layout()
    plt.savefig('hiv_phase_comparison.png', dpi=300)
...
def export_results_to_csv(results, filename_prefix="simulation", output_dir="output"):
    """
    ...
    if 'core' in results:
        # Export core quantum dynamics data
        core = results['core']
        filename = os.path.join(output_dir, f"{filename_prefix}_core_dynamics.csv")
        with open(filename, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Time', 'Fibonacci_Coherence', 'Regular_Coherence', 'Coherence_Ratio'])
            for i in range(len(core['times'])):
                writer.writerow([
                    core['times'][i],
                    core['fibonacci_coherence'][i],
                    core['regular_coherence'][i],
                    core['coherence_ratio'][i]
                ])
        print(f"Exported core dynamics data to {filename}")

    if 'golden_ratio' in results:
        # Export golden ratio optimization data
        gr = results['golden_ratio']
        filename = os.path.join(output_dir, f"{filename_prefix}_golden_ratio.csv")
        with open(filename, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Scaling_Factor', 'Coherence_Ratio'])
            for i in range(len(gr['scaling_factors'])):
                writer.writerow([
                    gr['scaling_factors'][i],
                    gr['coherence_ratios'][i]
                ])
        print(f"Exported golden ratio data to {filename}")

    if 'temperature' in results:
        # Export temperature resilience data
        temp = results['temperature']
        filename = os.path.join(output_dir, f"{filename_prefix}_temperature.csv")
        with open(filename, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([
                'Time', 'Temperature',
                'Regular_Integrity', 'Fibonacci_Integrity',
                'Regular_Resonance', 'Fibonacci_Resonance'
            ])
            for i in range(len(temp['times'])):
                writer.writerow([
                    temp['times'][i],
                    temp['temperatures'][i],
                    temp['regular_integrity'][i],
                    temp['fibonacci_integrity'][i],
                    temp['regular_resonance'][i],
                    temp['fibonacci_resonance'][i]
                ])
        print(f"Exported temperature data to {filename}")

    if 'extended' in results:
        # Export extended decay analysis data
        ext = results['extended']
        filename = os.path.join(output_dir, f"{filename_prefix}_extended.csv")
        with open(filename, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Time', 'Regular_Coherence', 'Fibonacci_Coherence', 'Coherence_Ratio'])
            for i in range(len(ext['times'])):
                writer.writerow([
                    ext['times'][i],
                    ext['regular_coherence'][i],
                    ext['fibonacci_coherence'][i],
                    ext['coherence_ratio'][i]
                ])
        print(f"Exported extended decay data to {filename}")

    if 'hiv_phases' in results:
        # Export HIV phase comparison data for each phase
        phases = results['hiv_phases']
        for phase in ['acute', 'art-treated', 'chronic-untreated']:
            if phase in phases:
                filename = os.path.join(output_dir, f"{filename_prefix}_phase_{phase}.csv")
                with open(filename, 'w', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(['Time', 'Regular_Coherence', 'Fibonacci_Coherence', 'Coherence_Ratio'])
                    times = np.linspace(0.07, params.t_total, 500)  # Use same times as in the function
                    for i in range(len(times)):
                        writer.writerow([
                            times[i],
                            phases[phase]['regular_coherence'][i],
                            phases[phase]['fibonacci_coherence'][i],
                            phases[phase]['ratio'][i]
                        ])
                print(f"Exported {phase} phase data to {filename}")

    if 'full_simulation' in results:
        # Export full simulation data
        sim = results['full_simulation']
        filename = os.path.join(output_dir, f"{filename_prefix}_full_simulation.csv")
        with open(filename, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([
                'Time', 'Temperature',
                'Regular_Grid_Integrity', 'Fibonacci_Grid_Integrity',
                'Coherence_Ratio'
            ])
            for i in range(len(sim['times'])):
                writer.writerow([
                    sim['times'][i],
                    sim['temperatures'][i],
                    sim['regular_grid_integrity'][i],
                    sim['fibonacci_grid_integrity'][i],
                    sim['coherence_ratios'][i]
                ])
        print(f"Exported full simulation data to {filename}")


def export_results_to_json(results, filename_prefix="simulation", output_dir="output"):
    """
    ...
    def convert_arrays(obj):
        if isinstance(obj, dict):
            # Skip figures and any non-serializable objects
            return {key: convert_arrays(value) for key, value in obj.items()
                    if key not in ['fig', 'figures'] and is_serializable(value)}
        elif isinstance(obj, list):
            return [convert_arrays(item) for item in obj if is_serializable(item)]
    ...
    def is_serializable(obj):
        try:
            ...
            # For more complex objects, check if it's a dictionary, list, or numpy array
    ...
    for category in categories:
        if category in results:
            try:
                # Skip non-serializable objects like matplotlib figures
                data = convert_arrays(results[category])

                # Ensure all matplotlib figures are removed
                if isinstance(data, dict) and 'figures' in data:
                    del data['figures']
                if isinstance(data, dict) and 'fig' in data:
                    del data['fig']

                filename = os.path.join(output_dir, f"{filename_prefix}_{category}.json")
                with open(filename, 'w') as file:
                    json.dump(data, file, indent=2)
                print(f"Exported {category} data to {filename}")
            except Exception as e:
                print(f"Error exporting {category} data: {e}")
                # Continue with other categories even if one fails
                continue

    # Export a combined summary
    summary = {}
    ...
    if 'core' in results:
        try:
            summary['core'] = {
                'max_advantage': float(results['core']['max_advantage']),
                'max_advantage_time': float(results['core']['max_advantage_time']),
                'sanctuary_time': float(results['core']['sanctuary_time']) if results['core'][
                    'sanctuary_time'] else None,
                'advantage_at_sanctuary': float(results['core']['advantage_at_sanctuary']) if results['core'][
                    'advantage_at_sanctuary'] else None
            }
        except Exception as e:
            print(f"Error including core in summary: {e}")

    # Add golden ratio summary
    if 'golden_ratio' in results:
        try:
            summary['golden_ratio'] = {
                'max_advantage': float(results['golden_ratio']['max_advantage']),
                'max_phi': float(results['golden_ratio']['max_phi'])
            }
        except Exception as e:
            print(f"Error including golden_ratio in summary: {e}")

    # Add temperature summary
    if 'temperature' in results:
        try:
            summary['temperature'] = {
                'max_temperature': float(results['temperature']['max_temperature']),
                'advantage_at_max_temp': float(results['temperature']['advantage_at_max_temp'])
            }
        except Exception as e:
            print(f"Error including temperature in summary: {e}")

    # Add extended summary
    if 'extended' in results:
        try:
            summary['extended'] = {
                'final_regular': float(results['extended']['final_regular']),
                'final_fibonacci': float(results['extended']['final_fibonacci']),
                'final_ratio': float(results['extended']['final_ratio'])
            }
        except Exception as e:
            print(f"Error including extended in summary: {e}")

    # Add HIV phases summary
    if 'hiv_phases' in results:
        try:
            summary['hiv_phases'] = {}
            for phase in ['acute', 'art-treated', 'chronic-untreated']:
                if phase in results['hiv_phases']:
                    summary['hiv_phases'][phase] = {
                        'max_ratio': float(results['hiv_phases'][phase]['max_ratio']),
                        'max_ratio_time': float(results['hiv_phases'][phase]['max_ratio_time'])
                    }
        except Exception as e:
            print(f"Error including HIV phases in summary: {e}")

    # Export summary
    try:
        filename = os.path.join(output_dir, f"{filename_prefix}_summary.json")
        with open(filename, 'w') as file:
            json.dump(summary, file, indent=2)
        print(f"Exported summary data to {filename}")
    except Exception as e:
        print(f"Error exporting summary: {e}")


def run_all_analyses_with_export(export_formats=None, output_dir="output", filename_prefix="simulation"):
    """
    Run all analyses and export results in specified formats.

    Parameters:
    -----------
    export_formats : list, optional
        List of export formats ('csv', 'json')
    output_dir : str
        Directory where output files will be saved
    filename_prefix : str
        Prefix for the output filenames
    ...
    try:
        ...
        # Extract key time series from simulation but don't include figures directly
        ...
        plot_results = plot_simulation_results(results)
        # Don't include figures in the results dictionary
    ...
    if 'csv' in export_formats:
        print(f"\nExporting results to CSV files in {output_dir}...")
        export_results_to_csv(all_results, filename_prefix=filename_prefix, output_dir=output_dir)

    if 'json' in export_formats:
        print(f"\nExporting results to JSON files in {output_dir}...")
        export_results_to_json(all_results, filename_prefix=filename_prefix, output_dir=output_dir)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run HIV Quantum Coherence Simulation')
    parser.add_argument('--export', nargs='+', choices=['csv', 'json'],
                        help='Export formats for simulation results')
    parser.add_argument('--plot', action='store_true',
                        help='Show plots after simulation')
    parser.add_argument('--output-dir', type=str, default='output',
                        help='Directory where output files will be saved')
    parser.add_argument('--output-prefix', type=str, default='simulation',
                        help='Prefix for output filenames')

    # Parse arguments
    args = parser.parse_args()

    # Run analyses with specified export formats and output directory
    export_formats = args.export if args.export else []
    all_results = run_all_analyses_with_export(
