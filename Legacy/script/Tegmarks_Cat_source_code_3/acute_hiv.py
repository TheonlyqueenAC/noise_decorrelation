...
log_message("Starting simulation script execution")

# Force matplotlib to use the Agg backend (non-interactive)
plt.switch_backend('Agg')

# Define the output directory path (use a local path in current directory)
output_dir = "./acute_il6_results"
...
# Simulation parameters based on IL_6_perturbation.py values
# Using the IL-6 Parameters from IL_6_perturbation.py
V_0 = 5.5  # Peak cytokine potential (adjusted based on IL-6 parameters)
Gamma_0 = 0.05  # Baseline decoherence rate
alpha_c = 0.12  # Scaling factor for cytokine-induced decoherence (adjusted based on IL-6 parameters)

# IL-6 Parameters from IL_6_perturbation.py
IL6_CONCENTRATION_ACUTE = 9.8  # Acute HIV infection IL-6 level (pg/mL)
IL6_COUPLING_STRENGTH_ACUTE = 0.28  # Coupling strength in acute phase

# Target time points for visualization
target_times = [0, 1.5, 2.5, 3.0]
target_frames = [int(t / dt) for t in target_times]
log_message(f"Target frames for visualization: {target_frames}")

# Create spatial grids
log_message("Creating spatial grids")
...
log_message("Generating Fibonacci grid")
...
log_message(f"Initial normalization - Regular: {initial_norm_reg:.6f}, Fibonacci: {initial_norm_fib:.6f}")


# Initialize cytokine field with IL-6 perturbation parameters for acute HIV
def initialize_il6_perturbation():
    """
    Initialize cytokine field for acute HIV with IL-6 perturbation parameters
    Based on modulation pattern in IL_6_perturbation.py
    """
    log_message("Initializing IL-6 perturbation field")
    cytokines = np.zeros((N_z, N_r))

    # Anatomically accurate initialization based on IL-6 distribution model
    for i in range(N_z):
        for j in range(N_r):
            ...
            # IL-6 in acute phase has specific pattern described in IL_6_perturbation.py
            # "Initially high, then oscillating decline"
            if outer_dist < 0.35:  # Near outer boundary
                # Gaussian profile with less extensive spread than cytokine storm
                cytokines[i, j] = 0.75 * np.exp(-(center_dist ** 2) / 0.6)
            else:
                # Faster decay toward inner regions - IL-6 is more localized than TNF-α
                cytokines[i, j] = 0.35 * np.exp(-((outer_dist - 0.35) / 0.4) ** 2) * np.exp(-(center_dist ** 2) / 0.75)

    # Add hotspots with characteristic IL-6 pattern
    hotspot_count = 3
    hotspot_locations = [
        (int(N_z * 0.35), int(N_r * 0.85)),  # Lower region near outer boundary
        (int(N_z * 0.65), int(N_r * 0.83)),  # Upper region near outer boundary
        (int(N_z * 0.5), int(N_r * 0.75))  # Middle region with deeper penetration
    ]

    for h_i, h_j in hotspot_locations:
        hotspot_size = 7 + np.random.randint(3)  # Moderate size for IL-6 pattern
        hotspot_intensity = 0.85  # High but not as extreme as cytokine storm
        for i in range(N_z):
            for j in range(N_r):
                dist_sq = ((i - h_i) / hotspot_size) ** 2 + ((j - h_j) / hotspot_size) ** 2
                if dist_sq < 1.0:
                    # Gaussian intensity profile
                    intensity_factor = hotspot_intensity * np.exp(-1.8 * dist_sq)
                    cytokines[i, j] = min(1.0, cytokines[i, j] + intensity_factor)

    # Apply a global scaling to match IL6_CONCENTRATION_ACUTE parameter
    # Scale to maintain 0-1 range while representing the relative IL-6 concentration
    cytokines = np.clip(cytokines * (IL6_CONCENTRATION_ACUTE / 10.0) / 1.0, 0, 1)

    return cytokines


# Generate initial cytokine field with IL-6 perturbation parameters
C = initialize_il6_perturbation()

# Potentials with cytokine influence
log_message("Calculating initial potentials")
...
# Modified decoherence term with IL-6 parameters
def calculate_decoherence(cytokine_field, baseline_rate, alpha, state="il6_acute"):
    """
    Calculate decoherence rate with IL-6 specific parameters for acute HIV
    Based on IL_6_perturbation.py
    """
    if state == "il6_acute":
        # IL-6 in acute phase has specific effect on decoherence
        alpha_modified = alpha * IL6_COUPLING_STRENGTH_ACUTE * 4.0  # Adjust coupling strength as scaling factor
        return baseline_rate * np.exp(alpha_modified * cytokine_field)
    elif state == "cytokine_storm":
        # Cytokine storm parameters for comparison
        alpha_modified = alpha * 1.6
        saturated_field = np.tanh(1.5 * cytokine_field)
        return baseline_rate * np.exp(alpha_modified * saturated_field)
    elif state == "acute":
        # Standard acute HIV parameters
        alpha_modified = alpha * 1.3
        return baseline_rate * np.exp(alpha_modified * cytokine_field)
    elif state == "study_volunteer":
...
# Initial decoherence rates with IL-6 parameters for acute HIV
log_message("Calculating initial decoherence rates")
Gamma_cytokine = calculate_decoherence(C, Gamma_0, alpha_c, state="il6_acute")

# Store initial coherence values for adaptive threshold
initial_max_prob_reg = np.max(np.abs(Psi_reg) ** 2)
initial_max_prob_fib = np.max(np.abs(Psi_fib) ** 2)
# Coherence history tracking
coherence_history_reg = []
coherence_history_fib = []
coherence_history_window = 10  # Window size for IL-6 acute phase


# Event horizon threshold for IL-6 perturbation
def calculate_event_horizon(Psi, initial_max_prob, coherence_level, grid_type="regular", min_threshold=0.05,
                            max_threshold=0.2):
    """
    Event horizon detection adjusted for IL-6 perturbation in acute HIV
    """
    prob_density = np.abs(Psi) ** 2
    current_max_prob = np.max(prob_density)

    # IL-6 specific threshold parameters
    if grid_type == "regular":
        # Regular grid with IL-6 parameters
        threshold_ratio = max(min_threshold, min(max_threshold, coherence_level * 0.22))
    else:
        # Fibonacci grid with IL-6 parameters
        threshold_ratio = max(min_threshold, min(max_threshold, coherence_level * 0.19))
...
def evolve_wavefunction(Psi, V, Gamma, dr, dz, dt, R_grid):
    ...
    if norm > 0:
        ...
        log_message("WARNING: Zero norm encountered during evolution!")

    return Psi_new


# Store probability densities and calculated metrics
log_message("Initializing data storage for simulation")
Psi_reg_list = []
Psi_fib_list = []
horizon_reg_list = []
horizon_fib_list = []
...
# Initial state
log_message("Storing initial state data")
...
# Save the initial state visualization
viz_start_time = time.time()
log_message("Creating initial state visualization")
try:
    ...
    ax3.set_title(f'Cytokine Field (t=0.0) - IL-6 Acute HIV')
    ...
    save_path = os.path.join(output_dir, f"acute_il6_t0.0_{timestamp_str}.png")
    plt.savefig(save_path, dpi=300)
    plt.close()
    log_message(f"Initial state visualization saved to {save_path}")
except Exception as e:
    log_message(f"Error creating initial visualization: {e}")

viz_end_time = time.time()
log_message(f"Initial visualization completed in {viz_end_time - viz_start_time:.2f} seconds")

# Time evolution loop
evolution_start_time = time.time()
log_message("Starting time evolution simulation for acute HIV with IL-6 perturbation...")
for step in range(1, time_steps + 1):
    step_start_time = time.time()
    current_time = step * dt

    # IL-6 cytokine dynamics from IL_6_perturbation.py
    # "Initially high, then oscillating decline"
    ...
    # Time-dependent modulation described in IL_6_perturbation.py
    # "Initially high, then oscillating decline"
    time_factor = 1.0 + 0.3 * np.sin(2.0 * current_time) * np.exp(-0.4 * current_time)

    # Diffusion and decay rates reflecting IL-6 behavior
    diffusion_rate = 0.04  # Moderate diffusion
    decay_rate = 0.018 * time_factor  # Time-dependent decay rate

    # Small fluctuations
    fluctuation_amplitude = 0.008  # Small random fluctuations
    fluctuations = fluctuation_amplitude * (np.random.random(C.shape) - 0.5)

    # Update cytokine field with IL-6 dynamics
    ...
    # State-specific decoherence calculation with IL-6 parameters
    Gamma_cytokine = calculate_decoherence(C, Gamma_0, alpha_c, state="il6_acute")
    ...
    if step % 10 == 0 or step in target_frames:
        ...
        if step % 50 == 0:
            ...
            log_message(
                f"Completed step {step}/{time_steps} ({step / time_steps * 100:.1f}%) - Step time: {step_time:.2f}s")
            log_message(f"  Elapsed time: {elapsed_time:.2f}s, Est. remaining: {estimated_remaining:.2f}s")
            log_message(f"  Time: {current_time:.2f}")
            log_message(f"  Standard variance: {disp_reg['variance']:.4f}")
            log_message(f"  Fibonacci variance: {disp_fib['variance']:.4f}")
            log_message(f"  Coherence standard: {coh_reg:.4f}")
            log_message(f"  Coherence fibonacci: {coh_fib:.4f}")
            log_message(f"  Rolling coherence reg: {rolling_coherence_reg:.4f}")
            log_message(f"  Rolling coherence fib: {rolling_coherence_fib:.4f}")
            log_message(f"  Normalization (reg, fib): ({post_norm_reg:.6f}, {post_norm_fib:.6f})")
    ...
    if step in target_frames:
        ...
        try:
            viz_start_time = time.time()
            log_message(f"Creating visualization for t = {target_time}")
            ...
            ax3 = fig.add_subplot(gs[0, 2])
            cont3 = ax3.contourf(Z, R, C, levels=20, cmap='plasma')
            ax3.set_title(f'Cytokine Field (t={target_time:.1f}) - IL-6 Acute HIV')
            ax3.set_xlabel('Axial Position (z)')
            ax3.set_ylabel('Radial Position (r)')
            plt.colorbar(cont3, ax=ax3, label='Concentration')
            ...
            plt.tight_layout()
            timestamp_str = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
            save_path = os.path.join(output_dir, f"acute_il6_t{target_time:.1f}_{timestamp_str}.png")
            plt.savefig(save_path, dpi=300)
            plt.close()

            viz_end_time = time.time()
            log_message(
                f"Visualization for t={target_time} saved to {save_path} in {viz_end_time - viz_start_time:.2f} seconds")

        except Exception as e:
            log_message(f"Error creating visualization for t={target_time}: {e}")

evolution_end_time = time.time()
evolution_duration = evolution_end_time - evolution_start_time
log_message(f"Time evolution completed in {evolution_duration:.2f} seconds ({evolution_duration / 60:.2f} minutes)")
...
integrated_coherence_reg = integrate.trapezoid(coherence_reg_array, times_array)
integrated_coherence_fib = integrate.trapezoid(coherence_fib_array, times_array)
...
log_message("Performing spectral analysis of coherence fluctuations")
...
log_message(f"Metrics calculation completed in {metrics_end_time - metrics_start_time:.2f} seconds")
# Generate summary figures for the entire simulation
log_message("Generating summary visualizations")
summary_viz_start_time = time.time()
try:
    ...
    ax1.set_title('Quantum Coherence Preservation - IL-6 Acute HIV')
    ax1.legend()
    ax1.grid(True)
    ...
    plt.tight_layout()
    timestamp_str = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    summary_path = os.path.join(output_dir, f"acute_il6_summary_{timestamp_str}.png")
    plt.savefig(summary_path, dpi=300)
    plt.close()
    log_message(f"Summary visualization saved to {summary_path}")
    ...
    ax3.set_title(f'Final Cytokine Field (t={time_list[-1]:.1f}) - IL-6 Acute HIV')
    ax3.set_xlabel('Axial Position (z)')
    ax3.set_ylabel('Radial Position (r)')
    plt.colorbar(cont3, ax=ax3, label='Concentration')
    ...
    plt.tight_layout()
    timestamp_str = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    final_path = os.path.join(output_dir, f"acute_il6_final_state_{timestamp_str}.png")
    plt.savefig(final_path, dpi=300)
    plt.close()
    log_message(f"Final state visualization saved to {final_path}")

except Exception as e:
    log_message(f"Error creating summary visualizations: {e}")

summary_viz_end_time = time.time()
log_message(f"Summary visualizations completed in {summary_viz_end_time - summary_viz_start_time:.2f} seconds")

# Save numerical results as JSON for later analysis and comparison
log_message("Saving simulation results to JSON")
results = {
    'simulation_parameters': {
        ...
        'IL6_concentration': IL6_CONCENTRATION_ACUTE,
        'IL6_coupling_strength': IL6_COUPLING_STRENGTH_ACUTE
    },
    'coherence_metrics': {
    ...
    'performance': {
        ...
        'evolution_time': evolution_duration,
        'metrics_calculation_time': metrics_end_time - metrics_start_time,
        'summary_visualization_time': summary_viz_end_time - summary_viz_start_time
    },
    "final_coherence_fibonacci": coherence_fib_list[-1],
    "final_coherence_regular": coherence_reg_list[-1],
...
timestamp_str = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
json_path = os.path.join("script/Core Scripts/comparison_results", "acute_hiv_results.json")
try:
    with open(json_path, 'w') as f:
        json.dump(results, f, indent=4)
    log_message(f"Results saved to {json_path}")
except Exception as e:
    log_message(f"Error saving results to JSON: {e}")
# Save coherence & variance arrays to .npz for plotting
timestamp_str = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
npz_path = os.path.join(output_dir, "acute_IL6_results.npz")
np.savez(npz_path,
...
log_message(f"\nAcute HIV with IL-6 perturbation simulation completed successfully!")
log_message(f"Total simulation time: {total_duration:.2f} seconds ({total_duration / 60:.2f} minutes)")
log_message(f"Final coherence - Regular: {coherence_reg_list[-1]:.4f}, Fibonacci: {coherence_fib_list[-1]:.4f}")
log_message(f"Coherence advantage: {coherence_advantage:.2f}%")
log_message(f"Final variance - Regular: {final_var_reg:.4f}, Fibonacci: {final_var_fib:.4f}")
log_message(f"Dominant frequency - Regular: {dom_freq_reg:.4f} Hz, Fibonacci: {dom_freq_fib:.4f} Hz")
